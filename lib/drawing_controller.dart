...
class DrawingController extends WhiteboardController {
  ...
  onPanUpdate(Offset position) {
    ...
    if (_newLine) {
      if (_chunker != null &&
          lastPan != null &&
          DateTime.now().difference(lastPan ?? DateTime.now()).inMilliseconds <
        ...
        this.draw.lines.add(new Line(
              ...
              duration: DateTime.now()
                  .difference(lastPan ?? DateTime.now())
                  .inMilliseconds,
            ));
      }
    ...
    if (this.draw.lines.last.points.length > 2 &&
        lastPan != null &&
        ((lastPan?.millisecond ?? DateTime.now().millisecond) -
    ...
    if (this.draw.lines.last.points.length == 0 ||
      ...
      this.draw.lines.last.duration = DateTime.now()
          .difference(firstPointTime ?? DateTime.now())
          .inMilliseconds;
